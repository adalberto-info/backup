	...
	IF EXISTS (SELECT 1 FROM DADOS_FECHAMENTO_RESSARCIMENTO A WHERE A.COD_FECHAMENTO = @COD_FECHAMENTO) 
		...
		IF EXISTS (SELECT 1 FROM RESSARCIMENTO_FILIAIS A WHERE A.COD_FECHAMENTO = @COD_FECHAMENTO)		
			...
			SELECT C.EMPRESA, C.COD_FILIAL, A.FILIAL, A.NOME_CLIFOR, A.NF_SAIDA, ISNULL(A.SERIE_NF,''), A.CHAVE_NFE, A.EMISSAO, A.VALOR_TOTAL, J.PRODUTO,
				   ...
				   B.UNIDADE, B.PRECO_UNITARIO, B.DESCRICAO_ITEM, '' AS NUMERO_MODELO_FISCAL
			FROM FATURAMENTO A
			...
			SELECT A.COD_FILIAL, A.NF_SAIDA, A.SERIE_NF, A.ITEM, A.TAMANHO, A.QTDE_VENDIDA
			FROM @TMP_SAIDA A

			OPEN CUR_SAIDA

			FETCH NEXT FROM CUR_SAIDA INTO @COD_FILIAL, @NF_SAIDA, @SERIE_NF, @ITEM, @TAMANHO, @QTDE_VENDIDA
			...
			BEGIN
				...
				insert into CONTROLE_ENT_RESSARCIMENTO (EMPRESA, COD_FILIAL, FILIAL, NOME_CLIFOR, NF_ENTRADA, SERIE_NF, CHAVE_NFE, RECEBIMENTO,
				...
				SELECT TOP 1 C.EMPRESA, C.COD_FILIAL, A.FILIAL, A.NOME_CLIFOR, A.NF_ENTRADA, ISNULL(A.SERIE_NF,''), A.CHAVE_NFE, A.RECEBIMENTO, 
					   ...
					   0 AS SALDO_NF_ENTRADA, B.REFERENCIA, B.DESCRICAO_ITEM, J.NUMERO_MODELO_FISCAL
				FROM ENTRADAS A 
				...
				AND NOT EXISTS (SELECT X.EMPRESA, X.COD_FILIAL, X.NF_ENTRADA, X.SERIE_NF, X.ITEM, X.COR_ITEM, X.TAMANHO
				FROM CONTROLE_ENT_RESSARCIMENTO X
				WHERE A.EMPRESA = X.EMPRESA
				AND C.COD_FILIAL = X.COD_FILIAL
				AND B.NF_ENTRADA = X.NF_ENTRADA 
				AND B.SERIE_NF = X.SERIE_NF
				AND B.CODIGO_ITEM = X.ITEM
				AND B.COR_ITEM = X.COR_ITEM
				AND B.TAMANHO = X.TAMANHO) 
			...
			INSERT INTO CONTROLE_SAI_RESSARCIMENTO (EMPRESA, COD_FILIAL, FILIAL, NOME_CLIFOR, NF_SAIDA, SERIE_NF, CHAVE_NFE, EMISSAO, VALOR_TOTAL, ITEM,
									COR_ITEM, TAMANHO, QTDE_VENDIDA, ALIQUOTA, BASE_ICMS_ST, ICMS_EFETIVO, ICMS_PRESUMIDO,
									BASE_ICMS_PRESUMIDO, VALOR_ISENTO, INDICA_CONSUMIDOR_FINAL, CODIGO_FISCAL_OPERACAO,
									NF_ENTRADA_RELACIONADA, SERIE_NF_RELACIONADA, NOME_CLIFOR_RELACIONADA, RECEBIMENTO_RELACIONADA,
									QTDE, CHAVE_NFE_SUBSTITUTO, SEQ_ITEM_SUBSTITUTO, COD_FECHAMENTO, UNIDADE, PRECO_UNITARIO, DESCRICAO_ITEM, NUMERO_MODELO_FISCAL)
			...
			insert into @TMP_ENTRADA (EMPRESA, COD_FILIAL, FILIAL, NOME_CLIFOR, NF_ENTRADA, SERIE_NF, CHAVE_NFE, RECEBIMENTO,
									  VALOR_TOTAL, ITEM, COR_ITEM, TAMANHO, QTDE_ENTRADA, ALIQUOTA, BASE_ICMS_ST,
									  ICMS_EFETIVO, ICMS_PRESUMIDO, BASE_ICMS_PRESUMIDO, VALOR_ISENTO,
									  SEQ_ITEM_SUBTITUTO, CHAVE_NFE_SUBSTITUTO, CODIGO_FISCAL_OPERACAO,
									  NATUREZA, SALDO_NF_ENTRADA, REFERENCIA, DESCRICAO_ITEM, NUMERO_MODELO_FISCAL) 
			SELECT C.EMPRESA, C.COD_FILIAL, A.FILIAL, A.NOME_CLIFOR, A.NF_ENTRADA, ISNULL(A.SERIE_NF,''), A.CHAVE_NFE, A.RECEBIMENTO, 
				   A.VALOR_TOTAL, B.CODIGO_ITEM, LEFT(ISNULL(B.REFERENCIA_ITEM, ''),10) AS COR_ITEM, 
				   B.SUB_ITEM_TAMANHO AS TAMANHO, B.QTDE_ITEM, D.ALIQUOTA, D.BASE_ICMS_SUBSTITUTO,
				   F.VALOR_IMPOSTO AS ICMS_EFETIVO, D.ICMS_PRESUMIDO, D.BASE_ICMS_SUBSTITUTO AS BASE_ICMS_PRESUMIDO, 
				   CASE WHEN G.TRIBUT_ICMS IN ('40','41') THEN B.VALOR_ITEM ELSE 0.00 END AS VALOR_ISENTO, 
			...
			--Inserindo na tabela CONTROLE_ENT_RESSARCIMENTO...
			insert into CONTROLE_ENT_RESSARCIMENTO (EMPRESA, COD_FILIAL, FILIAL, NOME_CLIFOR, NF_ENTRADA, SERIE_NF, CHAVE_NFE, RECEBIMENTO,
									  VALOR_TOTAL, ITEM, COR_ITEM, TAMANHO, QTDE_ENTRADA, ALIQUOTA, BASE_ICMS_ST,
									  ICMS_EFETIVO, ICMS_PRESUMIDO, BASE_ICMS_PRESUMIDO, VALOR_ISENTO,
									  SEQ_ITEM_SUBTITUTO, CHAVE_NFE_SUBSTITUTO, CODIGO_FISCAL_OPERACAO,
									  NATUREZA, SALDO_NF_ENTRADA, REFERENCIA, DESCRICAO_ITEM, NUMERO_MODELO_FISCAL) 
			select EMPRESA, COD_FILIAL, FILIAL, NOME_CLIFOR, NF_ENTRADA, SERIE_NF, CHAVE_NFE, RECEBIMENTO,
									  VALOR_TOTAL, ITEM, COR_ITEM, TAMANHO, QTDE_ENTRADA, ALIQUOTA, BASE_ICMS_ST,
									  ICMS_EFETIVO, ICMS_PRESUMIDO, BASE_ICMS_PRESUMIDO, VALOR_ISENTO,
									  SEQ_ITEM_SUBTITUTO, CHAVE_NFE_SUBSTITUTO, CODIGO_FISCAL_OPERACAO,
									  NATUREZA, SALDO_NF_ENTRADA, REFERENCIA, DESCRICAO_ITEM, NUMERO_MODELO_FISCAL
