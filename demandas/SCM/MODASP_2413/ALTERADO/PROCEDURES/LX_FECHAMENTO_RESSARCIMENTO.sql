	...
	IF EXISTS (SELECT 1 FROM DADOS_FECHAMENTO_RESSARCIMENTO A WHERE A.COD_FECHAMENTO = @COD_FECHAMENTO) 
		...
		IF EXISTS (SELECT 1 FROM RESSARCIMENTO_FILIAIS A WHERE A.COD_FECHAMENTO = @COD_FECHAMENTO)		
			...
			INSERT INTO @TMP_SAIDA (EMPRESA, COD_FILIAL, FILIAL, NOME_CLIFOR, NF_SAIDA, SERIE_NF, CHAVE_NFE, EMISSAO, VALOR_TOTAL, ITEM,
									...
									NUMERO_MODELO_FISCAL, ITEM_IMPRESSAO)
			SELECT C.EMPRESA, C.COD_FILIAL, A.FILIAL, A.NOME_CLIFOR, A.NF_SAIDA, ISNULL(A.SERIE_NF,''), A.CHAVE_NFE, A.EMISSAO, A.VALOR_TOTAL, J.PRODUTO,
				   ...
				   B.UNIDADE, B.PRECO_UNITARIO, B.DESCRICAO_ITEM, '' AS NUMERO_MODELO_FISCAL, B.ITEM_IMPRESSAO
			FROM FATURAMENTO A
			...
			DECLARE @IS_FINALIZOU_SAIDA [BIT], @IS_ACHOU_NF_ENTRADA [BIT]
			...
			SELECT A.COD_FILIAL, A.NF_SAIDA, A.SERIE_NF, A.ITEM, A.TAMANHO, A.COR_ITEM, A.QTDE_VENDIDA
			FROM @TMP_SAIDA A

			OPEN CUR_SAIDA

			FETCH NEXT FROM CUR_SAIDA INTO @COD_FILIAL, @NF_SAIDA, @SERIE_NF, @ITEM, @TAMANHO, @COR_ITEM, @QTDE_VENDIDA
			...
			BEGIN
				...
				IF @QTDE_ENTRADA_SALDO = 0
					...
					SELECT TOP 1 @NF_ENTRADA = A.NF_ENTRADA, @SERIE_NF_ENTRADA = ISNULL(A.SERIE_NF_ENTRADA,''), @QTDE_ENTRADA = B.QTDE_ITEM, 
					@NOME_CLIFOR_ENTRADA = A.NOME_CLIFOR, @RECEBIMENTO = A.RECEBIMENTO, @CHAVE_NFE_SUBSTITUTO = B.CHAVE_NFE_SUBSTITUTO,
					@SEQ_ITEM_SUBSTITUTO = B.SEQ_ITEM_SUBSTITUTO
					FROM ENTRADAS A 
				...
				WHERE COD_FILIAL = @COD_FILIAL AND NF_SAIDA = @NF_SAIDA AND SERIE_NF = @SERIE_NF AND ITEM = @ITEM AND TAMANHO = @TAMANHO AND COR_ITEM = @COR_ITEM
				...
				-----------------------------------------------------------------------------------------------------------------------------------------  
					BEGIN
						...
						SELECT TOP 1 @COD_FECHAMENTO, C.EMPRESA, C.COD_FILIAL, A.FILIAL, A.NOME_CLIFOR, A.NF_ENTRADA, ISNULL(A.SERIE_NF_ENTRADA,''), A.CHAVE_NFE, A.RECEBIMENTO, 
			...
			INSERT INTO CONTROLE_SAI_RESSARCIMENTO (EMPRESA, COD_FILIAL, FILIAL, NOME_CLIFOR, NF_SAIDA, SERIE_NF, CHAVE_NFE, EMISSAO,
									...
									BASE_ICMS_PRESUMIDO, VALOR_ISENTO, INDICA_CONSUMIDOR_FINAL, CODIGO_FISCAL_OPERACAO,
									NF_ENTRADA_RELACIONADA, SERIE_NF_RELACIONADA, NOME_CLIFOR_RELACIONADA, RECEBIMENTO_RELACIONADA,
									QTDE, CHAVE_NFE_SUBSTITUTO, SEQ_ITEM_SUBSTITUTO, COD_FECHAMENTO, UNIDADE, PRECO_UNITARIO, DESCRICAO_ITEM, NUMERO_MODELO_FISCAL, ITEM_IMPRESSAO)
			SELECT EMPRESA, COD_FILIAL, FILIAL, NOME_CLIFOR, NF_SAIDA, SERIE_NF, CHAVE_NFE, EMISSAO, VALOR_TOTAL, ITEM,
									...
									ISNULL(QTDE,0), CHAVE_NFE_SUBSTITUTO, SEQ_ITEM_SUBSTITUTO, @COD_FECHAMENTO, UNIDADE, PRECO_UNITARIO, DESCRICAO_ITEM,NUMERO_MODELO_FISCAL, ITEM_IMPRESSAO
			FROM @TMP_SAIDA
